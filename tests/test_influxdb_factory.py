...
@pytest.fixture(scope="session")
def wrong_db_login(docker_ip):
    db_login = {
        "url": f"http://{docker_ip}:8086/",
        "token": "WrongToken",
        "org": "WrongOrg",
        "bucket_name": "WrongBucket",
    }
    return db_login
...
@pytest.fixture(scope="session")
def run_influxdb(right_db_login, docker_services):
    docker_services.wait_until_responsive(timeout=2, pause=0, check=lambda: is_responsive(**right_db_login))


@pytest.mark.usefixtures("right_db_login", "wrong_db_login", "storage_engine", "run_influxdb")
class TestInfluxDBIntegration:
    def test_make_connection_is_not_none(self, right_db_login, storage_engine):
        influxdb = storage_engine.get_database_actions()
        db_connection = influxdb.make_connection(**right_db_login)
        assert db_connection is not None

    def test_make_connection_isinstance_InfluxDBClient(self, right_db_login, storage_engine):
        influxdb = storage_engine.get_database_actions()
        db_connection = influxdb.make_connection(**right_db_login)
        assert isinstance(db_connection, InfluxDBClient)

    def test_make_connection_args_check(self, right_db_login, storage_engine):
        influxdb = storage_engine.get_database_actions()
        db_connection = influxdb.make_connection(**right_db_login)
    ...
    def test_write_data_writes_data(self, storage_engine, right_db_login, capsys):
    ...
    def test_write_data_fails(self, storage_engine, right_db_login, capsys):
        # get dataframe to write
        test_data = pd.read_csv("tests/data/split/test_data_no_class.csv", header=0).copy()
    ...
    def test_write_data_returns_not_none(self, storage_engine, right_db_login):
        # get dataframe to write
        test_data = pd.read_csv("tests/data/split/test_data_no_class.csv", header=0).copy()
    ...
    def test_make_connection_fails(self, storage_engine, wrong_db_login, capsys):
        # get dataframe to write
        influxdb = storage_engine.get_database_actions()

        try:
            db_connection = influxdb.make_connection(**wrong_db_login)
            db_connection
        except ApiException:
            assert True
